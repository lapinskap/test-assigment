...
const {pathForDomain} = require('devcert/dist/constants');
const os = require('os');
...
const isSudoSession = () => execa.shell('sudo -n true').then(() => true).catch(() => false);

const alreadyProvisioned = (hostname) => devcert.configuredDomains().includes(hostname);
...
function getCert(hostname) {
  ...
  return new Promise(async (resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error('Timed out waiting for SSL certificate generation and trust.')), 30000);

    try {
      if (!alreadyProvisioned(hostname)) {
        if (process.stdin.isTTY) {
          if (!(await isSudoSession())) {
            console.warn(chalk.greenBright(`Creating a local development domain requires temporary administrative privileges.
Please enter the password for ${chalk.whiteBright(username)} on ${chalk.whiteBright(os.hostname())}.`));
          }
        } else {
          clearTimeout(timeout);
          return reject(new Error('Creating a local development domain requires an interactive terminal for the user to answer prompts. Run the development server (e.g. `yarn run watch:venia`) by itself in the terminal to continue.'));
        }
      }
      const certBuffers = await devcert.certificateFor(hostname, {getCaPath: true});
      clearTimeout(timeout);
      resolve({
        key: pathForDomain(hostname, 'private-key.key'),
        cert: pathForDomain(hostname, 'certificate.crt'),
      });
    } catch (e) {
      clearTimeout(timeout);
      reject(e);
...
function getUniqueDomainAndPorts(customName, addUniqueHash) {
  ...
  if (typeof customName === 'string') {
    ...
    try {
      ...
      console.warn(`Using default "${name}" prefix. Could not autodetect project name from package.json: `, e);
    }
  }

  const dirHash = createHash('md4'); // Using a hash of the current directory is a natural way of preserving
  ...
  const subdomain = addUniqueHash ? `${name}-${digest.slice(0, 5)}` : name; // Base64 truncated to 5 characters, stripped of special characters,
  ...
  return {
    uniqueSubdomain: subdomain.toLowerCase().replace(/[^a-zA-Z0-9]/g, '-').replace(/^-+/, ''),
    ports,
  };
}
...
async function configureHost({
  ...
  const {
    uniqueSubdomain,
    ports,
  } = getUniqueDomainAndPorts(exactDomain || subdomain, addUniqueHash);
  let hostname;
  ...
  try {
    ...
    throw Error(`Could not setup development domain: \n${e}`);
  }
}
