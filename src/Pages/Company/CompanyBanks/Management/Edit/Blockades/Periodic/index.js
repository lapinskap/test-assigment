...
import { Input, CardBody, CardTitle } from 'reactstrap';
...
export default () => {
  const [data, setData] = useState({ monthsConfig: mockData });
  ...
  return (
    <>
      <CardTitle>Blokada dla każdego miesiąca w roku:</CardTitle>
      <CardBody>
        <DateRange value={data.dateRange} onChange={(value) => onChange('dateRange', value)} />
      </CardBody>
      <CardTitle>Konfiguracja blokad indywidualnych w poszczególnych miesiącach:</CardTitle>
      <CardBody>
        <DataTable
          id="periodicBlockadesListing"
          noCards
          columns={columns(data.monthsConfig, onChange)}
          data={data.monthsConfig}
          filterable={false}
          defaultPageSize={12}
        />
      </CardBody>
