...
export default function PdfFormsListing({ match }) {
  const { companyId } = match.params;
  const companyName = useCompanyName();
  const [data, setData] = useState([]);
  const [fetchedData, setFetchedData] = useState(false);
  const getFormUrl = (id) => `/company/edit/${companyId}/subscriptions/pdf-forms/${id}`;

  const benefits = useBenefits(true, 'companyId', companyId, null, null, true);
  ...
  return (
    <>
      ...
      >
        <PageTitle
          heading={`Lista formularzy PDF dla firmy ${companyName} (ID: ${companyId})`}
          breadcrumbsHeading="Lista formularzy"
          breadcrumbs={[
        ...
        <DataLoading
          ...
          endpoint={`/pdf-forms?companyId=${companyId}&itemsPerPage=10000`}
